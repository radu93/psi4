...
#pragma omp parallel for schedule(guided, 4) num_threads(nthread)
    for (size_t MU = start; MU <= stop; MU++) {
        int rank = 0;
#ifdef _OPENMP
        rank = omp_get_thread_num();
...
void DFHelper::contract_metric_AO_core_symm(double* Qpq, double* Ppq, double* metp, size_t begin, size_t end) {
    // loop and contract
    size_t startind = symm_big_skips_[begin];
#pragma omp parallel for num_threads(nthreads_) schedule(guided, 4)
...
#pragma omp parallel for schedule(guided, 4) num_threads(nthreads_)
